...
void getCostMatrix(const Histogram& histogram, const Eigen::Vector3f& goal, const Eigen::Vector3f& position,
                   ...
                   Eigen::MatrixXf& cost_matrix, std::vector<uint8_t>& image_data, float fov_pitch_deg) {
  ...
  for (int e_index = 0; e_index < GRID_LENGTH_E; e_index++) {
    ...
    for (int z_index = 0; z_index < GRID_LENGTH_Z; z_index += step_size) {
      ...
      std::pair<float, float> costs = costFunction(p_pol, obstacle_distance, goal, position, velocity, cost_params,
                                                   closest_pt, is_obstacle_facing_goal,fov_pitch_deg);
...
std::pair<float, float> costFunction(const PolarPoint& candidate_polar, float obstacle_distance,
  ...
  double pitch_cost = cost_params.pitch_cost_param * (candidate_polar.e - facing_goal.e) * (candidate_polar.e - facing_goal.e);
...
void printHistogram(Histogram& histogram) {
  std::cout << "------------------------------------------Histogram------------------------------------------------\n";
  std::cout << "Grid_Lenght_e: " << GRID_LENGTH_E << " , Grid_Length_z: " << GRID_LENGTH_Z;
  for (int e = 0; e < GRID_LENGTH_E; e++) {
    for (int z = 0; z < GRID_LENGTH_Z; z++) {
...
float checkNewObstacles(Histogram& histogram) {
  float min_dist = 18;

  //Min dist zu obstacle im Zielbereich suchen
  for (int e = 14; e < 20; e++) {
    for (int z = 40; z < 50; z++) {
      if (histogram.get_dist(e,z) < min_dist && histogram.get_dist(e,z) > 0) {
        min_dist = histogram.get_dist(e,z);
      }
    }
  }
