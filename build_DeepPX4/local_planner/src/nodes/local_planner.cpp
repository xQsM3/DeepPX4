...
void LocalPlanner::determineStrategy() {
  ...
  if (!polar_histogram_.isEmpty()) {
    getCostMatrix(polar_histogram_, goal_, position_, velocity_, cost_params_, smoothing_margin_degrees_, closest_pt_,
                  max_sensor_range_, min_sensor_range_, cost_matrix_, cost_image_data_,fov_fcu_frame_[0].pitch_deg);
    ...
    // build search tree
    //fov_fcu_frame_[0].pitch_deg
    star_planner_->setFovSensor(fov_fcu_frame_[0].v_fov_deg);
...
std::string LocalPlanner::getPlanDist() {
  //Define fly over or around strategy
  //get minimum distance to obstacle in plan direction and check if a plath is currently planned
  int min_dist = checkNewObstacles(polar_histogram_); //Function from planner_functions
  int goal_dist = sqrt(pow(position_(1)-goal_(1),2) + pow(position_(0)-goal_(0),2));
  if (goal_dist < 2) {
  plan_dist = goal_dist;
  strategy = "normal"; }
  else if (plan_dist < 0 || plan_dist > 17) {
    plan_dist = min_dist;
    strategy = "normal";
    no_strategy = true; }
  else {
    int goal_prog = prev_goal_dist - goal_dist;
    prev_goal_dist = goal_dist;
    plan_dist = plan_dist - goal_prog;
    if (no_strategy) {
      if (plan_dist > 7) { strategy = "vertical"; }
      else if (plan_dist > 2) { strategy = "horizontal"; }
...
avoidanceOutput LocalPlanner::getAvoidanceOutput(double close_obst) const {
  avoidanceOutput out;
  ...
  float sensor_speed = (-b + std::sqrt(b * b - 4 * a * c)) / (2 * a);
  float speed = std::isfinite(mission_item_speed_) ? mission_item_speed_ : px4_.param_mpc_xy_cruise;


  //Dist Speed 
  float dist_speed = dist_prop * close_obst;
  float limited_speed = std::min(dist_speed, sensor_speed);

  
  //Distance to goal dependency
  float dist_goal_ = sqrt(pow(position_(2) - goal_(2),2) + pow(position_(1)-goal_(1),2) + pow(position_(0)-goal_(0),2));
  float goal_speed;
  if (dist_goal_>0.2) {
    goal_speed = dist_goal_ * goal_prop;}
  else {
    goal_speed = 0;} // */


  //overall max_speed
  float max_speed = std::min(sensor_speed, speed);  //Baseline speed

  //float max_speed = std::min(speed, limited_speed); //Dist speed alone
  //max_speed = std::min(goal_speed, max_speed); //Dist speed + goal speed  // */
  //float  max_speed = std::min(goal_speed, sensor_speed); //goal_speed alone
  //max_speed = std::min(max_speed, speed); //goal_speed alone */
