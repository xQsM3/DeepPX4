...
class LocalPlannerNodelet : public nodelet::Nodelet {
  ...
  **/
  ...
  double close_obst_left_;
  double close_obst_right_;
  double close_obst_; 
 ...
 private:
  ...
  ros::Subscriber rec_velocity_sub_left_;
  ros::Subscriber rec_velocity_sub_right_;
  ...
  **/
  ...
  //void recVelocityCallback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg1, const sensor_msgs::PointCloud2ConstPtr& cloud_msg2);
  void recVelocityCallbackLeft(const sensor_msgs::PointCloud2ConstPtr& cloud_msg);

  /**
  * @brief     callaback for closest point of the left camera
  * @param[in] cloud_msg
  **/

  void recVelocityCallbackRight(const sensor_msgs::PointCloud2ConstPtr& cloud_msg);

  /**
  * @brief     callaback for closest point of the right camera
  * @param[in] cloud_msg
  **/

  double closeObstFun(const sensor_msgs::PointCloud2ConstPtr& cloud_msg);

  /**
  * @brief	callback to identify closest point in point cloud
  * @param[in]  lat_dist;
  * @param[out] out;
  **/
  ...
  * @param[in]  name of parameter which shall be changed
  * @param[in]  new value of the parameter
  **/
  void communication_controller(std::string para_name, float para_value) const;

  /**
  * @brief	sets the yaw-pitch-ratio to one of three predetermined settings
  * @param[in]  distance to the obstacle on the path (0 = no obstacle)
  **/
  void set_yaw_pitch(std::string strategy) const;
};
}
