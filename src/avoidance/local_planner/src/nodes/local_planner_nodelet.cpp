...
//Ende meiner Bibliotheken

//px4 avoid standard bibliotheken
...
void LocalPlannerNodelet::InitializeNodelet() {
  ...
  rec_velocity_sub_left_ = nh_.subscribe("/camera_left/depth/points", 1, &LocalPlannerNodelet::recVelocityCallbackLeft, this);
  rec_velocity_sub_right_ = nh_.subscribe("/camera_right/depth/points", 1, &LocalPlannerNodelet::recVelocityCallbackRight, this);
...
void LocalPlannerNodelet::recVelocityCallbackLeft(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
	close_obst_left_ = closeObstFun(cloud_msg);
}

void LocalPlannerNodelet::recVelocityCallbackRight(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
	close_obst_right_ = closeObstFun(cloud_msg);
}

double LocalPlannerNodelet::closeObstFun(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
	...
	//determine lateral distance of all points
...
void LocalPlannerNodelet::threadFunction() {
  float cost_prop = 1.0;  //proportionality factor: cost_param = cruise_velocity * obst_prop
  float param_value = 0;  //initialization of the value for obstacle_cost_param
  std::string param_name = "obstacle_cost_param_"; //parameter which shall be changed
  while (!should_exit_) {
...

    {
      ...
      std::lock_guard<std::mutex> lock(waypoints_mutex_);
      close_obst_ = std::min(close_obst_left_, close_obst_right_);
      avoidanceOutput avoidance_output = local_planner_->getAvoidanceOutput(close_obst_);
      wp_generator_->setPlannerInfo(avoidance_output);
      param_value = cost_prop * avoidance_output.cruise_velocity;

      last_wp_time_ = ros::Time::now();
    }
    //obstacle cost adaption = min safety distance = f(v)
    //LocalPlannerNodelet::communication_controller(param_name, param_value);

    //Yaw/Pitch adaption
    std::string strategy = local_planner_->getPlanDist();
    LocalPlannerNodelet::set_yaw_pitch(strategy);
...
void LocalPlannerNodelet::communication_controller(std::string para_name, float para_value) const {
...
void LocalPlannerNodelet::set_yaw_pitch(std::string strategy) const {
  ...
  float pitch_high = 25;
   /*
  DeepPX4
  this is done now by parameter tuner, we dont need this
  if (strategy == "normal") {
  //normal Yaw/Pitch ratio
    LocalPlannerNodelet::communication_controller("yaw_cost_param_", yaw_norm);
    LocalPlannerNodelet::communication_controller("pitch_cost_param_", pitch_norm);
  }
  else if (strategy == "horizontal") {
  //low yaw high pitch --> horizontal evasion
    LocalPlannerNodelet::communication_controller("yaw_cost_param_", yaw_low);
    LocalPlannerNodelet::communication_controller("pitch_cost_param_", pitch_high);
  }
  else {
  //high yaw low pitch --> vertical evasion
